...
execute as @a[scores={clickStick=1}] if items entity @s weapon *[custom_data~{dominance:true}] at @s anchored eyes facing entity @p[team=CIS,distance=..10] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.6] rotated ~ 0 run effect give @p[team=CIS,distance=..10] instant_damage 1 0 true
