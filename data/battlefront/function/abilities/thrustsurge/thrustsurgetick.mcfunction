...
execute as @a[scores={clickStick=1}] if items entity @s weapon *[custom_data~{thrust:true}] at @s anchored eyes facing entity @p[team=REP,distance=..10] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.6] rotated ~ 0 unless block ~ ~-1 ~ air at @a[team=REP,distance=..10,limit=1] if items entity @s weapon.mainhand *[custom_data~{thrust:true}] run tp @s ^ ^1 ^1 facing entity @a[team=REP,distance=..10,limit=1]
