...
execute as @a[scores={clickStick=1}] if items entity @s weapon *[custom_data~{thrust:true}] at @s anchored eyes facing entity @p[team=REP,distance=..15] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.6] rotated ~ 0 unless block ~ ~-1 ~ air run damage @p[team=REP,distance=..15] 6 player_attack by @s
