...
execute as @a[scores={survCooldown=800..}] run scoreboard players set @s survCooldown 0
...
execute as @a[scores={survCooldown=1..}] if items entity @s weapon *[custom_data~{survivor:true}] run title @s actionbar ["",{"score":{"name":"*","objective":"secondsHelper"},"color":"red"},{"text":"/40 seconds","color":"red"}]
