...
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["restrict"]}}}] at @s unless block ~ ~-1 ~ air if entity @p[team=CIS,distance=..6] run scoreboard players set @p[team=CIS,distance=..6] abilitiesBlocked 100
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["restrict"]}}}] at @s unless block ~ ~-1 ~ air if entity @p[team=CIS,distance=..6] run tellraw @p[team=CIS,distance=..6] {"text":"Hit by Obi-Wan Kenobi's restrictive mind trick!","color":"red"}
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["restrict"]}}}] at @s unless block ~ ~-1 ~ air if entity @p[team=CIS,distance=..6] run tellraw @s ["",{"text":"Restricted the abilities of ","color":"green"},{"selector":"@a[distance=..6,team=CIS]"},{"text":".","color":"green"}]
