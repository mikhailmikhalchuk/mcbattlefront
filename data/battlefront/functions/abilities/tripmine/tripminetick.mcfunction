...
execute as @a[scores={clickStick=1,retrDuration=1..},nbt={SelectedItem:{tag:{Tags:["trip"]}}}] run tellraw @s {"text":"Cannot use item! Abilities are restricted.","color":"red"}
execute as @a[scores={clickStick=1,retrDuration=1..},nbt={SelectedItem:{tag:{Tags:["trip"]}}}] run scoreboard players set @s clickStick 0
