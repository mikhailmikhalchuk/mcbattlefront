...
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["repeater"]}}}] run data modify entity @e[type=arrow,tag=dcprojectile,limit=1] Owner set from entity @s UUID
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["repeater"]}}}] at @s run playsound minecraft:entity.arrow.shoot block @a ~ ~ ~
