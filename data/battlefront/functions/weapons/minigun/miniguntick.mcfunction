...
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["mini"]}}}] run data modify entity @e[type=arrow,tag=mgprojectile,limit=1] Owner set from entity @s UUID
execute as @a[scores={clickStick=1},nbt={SelectedItem:{tag:{Tags:["mini"]}}}] at @s run playsound minecraft:entity.arrow.shoot block @a ~ ~ ~
